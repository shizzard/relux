...
- **Status**: rejected (superseded by R013)
- **Created**: 2026-04-07
...
Superseded by R013 (Pure String Match). R013 reworks the operator system so value matching needs no `|` prefix at all: with assignment relocated to `:=`, the bare `=` (contains) and `?` (regex) infix operators serve value matching directly (`version ? (\d+)\.(\d+)\.(\d+)`), unifying the marker and statement forms instead of introducing a parallel `|=`/`|?` pair. R013 also resolves the purity concern this RFC raised -- a match failure is a fail-fast test panic, the same unwinding vehicle an impure `fn` body already uses, rather than a new "requires a test execution context" purity dimension.
