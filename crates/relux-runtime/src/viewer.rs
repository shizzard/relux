...
//! and baked into the binary at compile time. Consumers (the per-test
//! `event.html` emitter — added in a follow-up commit) decompress and
//! inline the JS into a `<script>` tag inside `event.html`.
//!
