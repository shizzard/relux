...
use crate::core::config::ReluxConfig;
use crate::diagnostics::ModulePath;
use crate::dsl::resolver::FsSourceLoader;
use crate::dsl::resolver::discover_test_modules;
...
pub fn build_source_loader(
    project_root: &std::path::Path,
) -> Box<dyn crate::dsl::resolver::SourceLoader> {
    let relux_dir = project_root.join(config::RELUX_DIR);
