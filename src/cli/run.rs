...
pub async fn cmd_run(matches: &clap::ArgMatches) {
    ...
    let progress = match matches.get_one::<String>("progress").map(|s| s.as_str()) {
        Some("plain") => crate::runtime::ProgressMode::Plain,
        Some("tui") => crate::runtime::ProgressMode::Tui,
        _ => crate::runtime::ProgressMode::Auto,
    };
    ...
    let exec = crate::runtime::execute(&suite, &run_ctx).await;
    let results = exec.results;

    // Summary
    let total_duration: std::time::Duration = results.iter().map(|r| r.duration).sum();
    crate::runtime::report::run_summary::write_run_summary(
        &run_dir,
        &run_id,
        &results,
        total_duration,
    ...
    // Report
    let report = crate::runtime::report::result::RunReport {
        results: &results,
        run_dir: &run_dir,
    ...
    crate::runtime::report::html::generate_run_summary(&run_dir, &results);

    // Optional artifact formats
    let suite_name = cfg.name.as_deref().unwrap_or("relux");
    if matches.get_flag("tap") {
        crate::runtime::report::tap::generate_tap(
            &run_dir,
            suite_name,
            &results,
    ...
    if matches.get_flag("junit") {
        crate::runtime::report::junit::generate_junit(
            &run_dir,
            suite_name,
            &results,
            &suite.tables.sources,
