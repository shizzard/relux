...
There is no `==` operator: `x == y` (adjacent `=`, no space between them) is a parse error. Use `x = y` to assert equality, `x := y` to bind. A space is not an error -- `x = = y` matches the literal pattern `= y` (leading `= `).
